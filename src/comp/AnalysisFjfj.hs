...
convAPackageToSAL errh flags apkg | not (hasDumpStrict flags DFdumpFjfj) =
...
extractInstance :: SExpr -> [SId]
extractInstance = DataGenerics.everything 
  (++)
  (DataGenerics.mkQ [] test)
  where 
    test sexpr@(SStructSel (SVar (SQId Nothing (SId st))) inst) =
      if "state" `isInfixOf` st then [inst]
      else []
    test _ = []
    
extractMeth :: SExpr -> [SId]
extractMeth = DataGenerics.everything 
  (++)
  (DataGenerics.mkQ [] test)
  where 
    test sexpr@(SApply (SVar (SQId (Just _) (SId meth_id))) _) =
...
-- actionMethodCalled :: SDefn -> [SId]
actionMethodCalled = DataGenerics.everything
    (++)
    (DataGenerics.mkQ [] test)
    --No need to do CDefT as we are pretypechecking?
    where
        -- test cdef@( SLet [SApply (SVar var@(SQId (Just (mod, tparam, params)) met)) rhs) = 
        test sexpr@(SLet [(_sid, STTuple [STVar (SQId (Just _) ty), _], expr)] _) = 
          if ty == modTypeId then
            -- Extract method name and instance
            [(head $ extractMeth expr, head $ extractInstance expr)]
            -- [head $ extractInstance expr]
          else 
            []
          ...
          -- not (isMethod id)
        test _ = []
        -- SStructSel (SVar (SQId Nothing (SId "state0"))) (SId "inst_a")
        -- SApply (SVar (SQId (Just (SId "CTX_RegN",[],[SIntLit 32])) (SId "meth_write"))